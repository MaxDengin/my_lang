...
pg = ParserGenerator(
    ...
    ['STRING', 'INTEGER', 'FLOAT', 'IDENTIFIER', 'BOOLEAN',
     'PLUS', 'MINUS', 'MUL', 'DIV',
     'IF', 'ELSE', 'AND', 'OR', 'NOT','WHILE', 'COLON', 'END', 'PRINT',
     '(', ')', '=', '==', '!=', '>=', '<=', '<', '>', '[', ']', ',',
     '{','}',
     '$end', 'NEWLINE',
    ...
    precedence=[
        ('left', ['=']),
        ('left', ['[',']',',']),
        ('left', ['END', 'IF', 'ELSE', 'NEWLINE', 'WHILE', 'PRINT']),
        ('left', ['AND', 'OR',]),
        ('left', ['NOT',]),
        ('left', ['==', '!=', '>=','>', '<', '<=',]),
        ('left', ['PLUS', 'MINUS',]),
...
@pg.production('expression : IF expression COLON statement NEWLINE END')
def expression_if_single_line(state, p):
    return If(condition=p[1],body=p[3])

@pg.production('expression : IF expression COLON statement ELSE COLON statement NEWLINE END')
